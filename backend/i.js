...
app.post(
  ...
  async (req, res) => {
    try {
      const { name, empUsername, empPassword, salary, empId } = req.body;

      if (!name || !empUsername || !empPassword || !empId) {
        return res.status(400).json({
          ok: false,
          error: 'Name, empUsername, empPassword, and empId are required',
        });
      }

      // Check if username or empId already exists
      const existingEmp = await Employee.findOne({
        $or: [{ username: empUsername }, { empId }],
      });
...
app.post(
  ...
  async (req, res) => {
    try {
      const { employeeId, name, empUsername, empPassword, salary, empId } =
        req.body;
      ...
      if (empUsername) updateData.username = empUsername;
      if (empPassword) updateData.password = empPassword;
